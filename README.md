...
- `ghcr.io/ripples-alive/derper:1.98.5`
...
Local builds use `build.sh`, which pushes `ripples/derper:1.98.5` to Docker
Hub for `linux/amd64` and `linux/arm64`.

GitHub Actions builds the same Dockerfile directly with Buildx and the build
argument `VERSION=1.98.5`. The workflow publishes
`ghcr.io/ripples-alive/derper:1.98.5` and `ghcr.io/ripples-alive/derper:latest`
on pushes to `main`, version tags such as `v1.98.5`, and manual
