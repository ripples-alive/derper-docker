...
- `ghcr.io/ripples-alive/derper:1.96.2`
...
Local builds use `build.sh`, which pushes `ripples/derper:1.96.2` to Docker
Hub for `linux/amd64` and `linux/arm64`.

GitHub Actions builds the same Dockerfile directly with Buildx and the build
argument `VERSION=1.96.2`. The workflow publishes
`ghcr.io/ripples-alive/derper:1.96.2` and `ghcr.io/ripples-alive/derper:latest`
on pushes to `main`, version tags such as `v1.96.2`, and manual
